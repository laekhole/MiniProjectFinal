...
<link rel="stylesheet"
	href="<c:url value='/resources/css/style-boardList.css' />">
...
#boardInsertForm input[type="text"], #boardInsertForm textarea {
	width: 100%;
	padding: 8px;
	font-size: 16px;
...
<body>
	...
	<form name="pageForm" id="pageForm"
		action="<c:url value='/board/list.do'/>" method="post">
	...
	<form name="mForm" id="mForm" action="<c:url value='/board/list.do'/>"
		method="post">
		<input type="hidden" name="pageNo" id="pageNo"
		...
		<div
			...
			<div
				...
				<label style="margin-right: 10px;">건수: </label> <select
					...
					<option value="10"
						${result.board.pageLength == 10 ? 'selected="selected"' : ''}>10건
						&darr;</option>
					<option value="20"
						${result.board.pageLength == 20 ? 'selected="selected"' : ''}>20건
						&darr;</option>
					<option value="50"
						${result.board.pageLength == 50 ? 'selected="selected"' : ''}>50건
						&darr;</option>
					<option value="100"
						${result.board.pageLength == 100 ? 'selected="selected"' : ''}>100건
						&darr;</option>
		...
		<div id="board">
			...
			<c:forEach var="board" items="${result.list}">
				<div class="post">
					<div class="post-meta">
						<div class="check-box">
							<input type="checkbox" class="chk-box" name="boardid"
								value="${board.boardid}">
						</div>
	...
	<div id="boardReplyForm">
		<h1>간단한 게시판 - 답글 달기</h1>
		<input type="hidden" id="boardid4" name="boardid4"> <input
...
function mailSend(){
	...
	console.log(title+"   "+boardid+"  "+writer_email);
	var param ={
			title: title,
			boardid: boardid,
	...
	console.log(param);
	fetch("<c:url value='/board/sendMail.do'/>", {
	    method: "POST",
	    headers: {
	        "Content-Type": "application/json; charset=UTF-8",
	    },
...
// 	       // alert("게시판 상세보기");
	      ...
	      const param ={
	       ...
	       .then((json) => {
	           if (json.status) {
	               // alert("정보 실시간 반영 성공!");
	  							...
	  							const email = json.boardView.writer_email;
	       ...
	       commentList(boardid); // 게시글에 맞는 댓글 띄우기
	    });
...
// 	    // alert("댓글 작성 js 함수 시작");
	...
	// R fetch 
	// 반복 시 계속 commentList(boardid)로 가자. commentList(boardid);
...
$(document).on("click", ".commentDeleteButton", function() {
	  ...
	  var boardid = $("#boardid1").val(); // 삭제 루틴에는 필요 없지만, 댓글 리스트 업데이트에 필요함
